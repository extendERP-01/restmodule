...
public class InventoryItemDao {
	...
	public ResponseEntity<JsonResponse<Object>> addItem(List<RestInventoryItemModel> restItemModel) {
		...
		if (restItemModel.get(0).getItemCategory() == null || restItemModel.get(0).getItemCategory() == "") {
			...
			validity = false;
		} else if (restItemModel.get(0).getServiceType() == null || restItemModel.get(0).getServiceType() == "") {
			resp.setMessage("ServiceType Required");
			validity = false;
		...
		try {
			...
			for (Object[] m : x) {
				RestInventoryItemModel item = new RestInventoryItemModel(m[0], m[1], m[2], m[3], m[4], m[5], m[6], m[7],
						m[8], m[9], m[10], m[11], m[12], null, null, null, null, null, null, null, null, null ,null, null, null, null, null ,null);
		...
		try {
			...
			for (Object[] m : x) {
				RestInventoryItemModel restItemModel = new RestInventoryItemModel(m[0], m[1], m[2], m[3], m[4], m[5],
						m[6], m[7], m[8], m[9], m[10], m[11], m[12], null, null, null, null, m[13], m[14], m[15],null, m[16] ,m[17], m[18],m[19], m[20], m[21], m[22]);
			...
			if (!x.isEmpty()) {
				for (Object[] m : x) {
					RestInventoryItemModel restItemModel = new RestInventoryItemModel(m[0], m[1], m[2], m[3], m[4],
							...
							m[16],null, m[17] ,m[18], m[19], m[20], m[21], m[22], m[23]);
					form.add(restItemModel);
				}
				resp.setBody(form);
			}
		...
		try {
			...
			for (Object[] m : x) {
				RestInventoryItemModel item = new RestInventoryItemModel(m[0], null,null,m[1], m[2], m[3], m[4], m[5], m[6],null,
						null, m[9], null, null, null, null, null, null, null, null, null, null, m[10], null, null, null, null ,null);
				restItemModel.add(item);
			}
