	...
	public ResponseEntity<JsonResponse<List<RestInventoryReqDetailsModel>>> getReqQuotData(
		...
		try {
			...
			for (Object[] m : x) {
				...
				RestInventoryReqDetailsModel user = new RestInventoryReqDetailsModel(m[0], m[1], validityDate,m[3], m[4],m[5],m[6],null,null,createdDate);
				meal.add(user);
			}
			if (x.get(0).length > 8) {
		...
		try {
		  ...
		  //  System.out.println("value for pop up in dao set id is for rfQ------------ : " + value);
			...
			for (Object[] m : x) {
				...
				RestInventoryReqDetailsModel table = new RestInventoryReqDetailsModel(m[0], m[1], validDate,m[3], m[4],m[5],m[6],null,null,null);
