...
//				System.out.println(values);
	...
	public ResponseEntity<JsonResponse<Object>> saveCustomerDiscount(DropDownModel id) {
		logger.info("Method : saveCustomerDiscount Dao starts");
		Boolean validity = true;
		JsonResponse<Object> resp = new JsonResponse<Object>();
		resp.setMessage("");
		resp.setCode("");
		...
		if (validity)
			try {
				String values = "SET @P_Customer='" + id.getKey() + "', @P_Discount=" + id.getName() + ";";
					entityManager.createNamedStoredProcedureQuery("salesCustomerRoutines")
					.setParameter("actionType", "saveDiscount")
					.setParameter("actionValue", values)
					.execute();
		...
		ResponseEntity<JsonResponse<Object>> response = new ResponseEntity<JsonResponse<Object>>(resp,
				HttpStatus.CREATED);
		logger.info("Method : saveCustomerDiscount Dao ends");
		return response;
	}
...
//		System.out.println("Response :"+response);
	...
	@SuppressWarnings("unchecked")
	public ResponseEntity<JsonResponse<DropDownModel>> customerDiscountView(String id) {
		logger.info("Method : customerDiscountView Dao starts");
		List<DropDownModel> form = new ArrayList<DropDownModel>();
		try {
			String value = "SET @p_customer='" + id + "';";
			System.out.println(value);
			List<Object[]> x = entityManager.createNamedStoredProcedureQuery("salesCustomerRoutines")
					.setParameter("actionType", "discountView").setParameter("actionValue", value).getResultList();
			
			for (Object[] m : x) {
				DropDownModel restSalesCustomerModel = new DropDownModel(m[0], m[1].toString());
				form.add(restSalesCustomerModel);
			}
		...
		ResponseEntity<JsonResponse<DropDownModel>> response = new ResponseEntity<JsonResponse<DropDownModel>>(
				resp, responseHeaders, HttpStatus.CREATED);
		logger.info("Method : customerDiscountView Dao ends");
//		System.out.println("Response :"+response);
		return response;
	}
