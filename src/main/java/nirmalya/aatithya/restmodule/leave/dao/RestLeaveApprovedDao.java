/**
 * Defines RestLeaveApprovedDao DAO
 *
 */
 ...
 */
@Repository
public class RestLeaveApprovedDao {
	
	Logger logger = LoggerFactory.getLogger(RestLeaveApprovedDao.class);
	@Autowired
	ServerDao serverDao;
	@Autowired
	EntityManager em;
	...
	public ResponseEntity<JsonResponse<List<DropDownModel>>> getALeaveType(String getALeaveType) {
		...
		logger.info("Method : DAO getALeaveType starts");
		
		List<DropDownModel> dropDownModel = new ArrayList<DropDownModel>();
		
		try {
			String value = "SET @p_leavetype=" + 0 + ";";
			List<Object[]> x = em.createNamedStoredProcedureQuery("leaveApprovedRoutines")
					.setParameter("actionType", getALeaveType)
		...
		logger.info("Method : DAO getALeaveType ends");	
		return response;
	}
	...
	public ResponseEntity<JsonResponse<List<RestLeaveApplyModel>>> getALApplyData(
		...
		try { 
			List<Object[]> x = em.createNamedStoredProcedureQuery("leaveApprovedRoutines")
					.setParameter("actionType", "viewALApplied")
	 ...
	 */
 	@SuppressWarnings("unchecked")
	public ResponseEntity<JsonResponse<RestLeaveApplyModel>> getLeaveApprovedById(String id,String action) {	
