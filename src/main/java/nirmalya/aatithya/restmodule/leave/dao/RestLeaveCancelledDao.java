/**
 * Defines RestLeaveCancelledDao DAO
 *
 */
 ...
 */
@Repository
public class RestLeaveCancelledDao {
	
	Logger logger = LoggerFactory.getLogger(RestLeaveCancelledDao.class);
	@Autowired
	ServerDao serverDao;
	@Autowired
	EntityManager em;
	...
	public ResponseEntity<JsonResponse<List<DropDownModel>>> getCLeaveType(String getCLeaveType) {	
		logger.info("Method : DAO getCLeaveType starts");
		List<DropDownModel> dropDownModel = new ArrayList<DropDownModel>();
		try {
			String value = "SET @p_leavetype=" + 0 + ";";
			List<Object[]> x = em.createNamedStoredProcedureQuery("leaveCancelledRoutines")
					.setParameter("actionType", getCLeaveType)
		...
		logger.info("Method : DAO getCLeaveType ends");	
		return response;
	}
	...
	public ResponseEntity<JsonResponse<List<RestLeaveApplyModel>>> getCLApplyData(
		...
		logger.info("Method : getCLApplyData details starts");
		...
		try {	 
			List<Object[]> x = em.createNamedStoredProcedureQuery("leaveCancelledRoutines")
					.setParameter("actionType", "viewCLApplied")
	 ...
	 */
 	@SuppressWarnings("unchecked")
	public ResponseEntity<JsonResponse<RestLeaveApplyModel>> getLeaveCancelById(String id,String action) {
