/**
 * Defines RestLeavePendingDao DAO
 *
 */
 ...
 */
@Repository
public class RestLeavePendingDao {
	
	Logger logger = LoggerFactory.getLogger(RestLeavePendingDao.class);
	@Autowired
	ServerDao serverDao;
	@Autowired
	EntityManager em;
	...
	public ResponseEntity<JsonResponse<List<DropDownModel>>> getPLeaveType(String getPLeaveType) {
		logger.info("Method : DAO getPLeaveType starts");
		List<DropDownModel> dropDownModel = new ArrayList<DropDownModel>();
		try {
			String value = "SET @p_leavetype=" + 0 + ";";
			List<Object[]> x = em.createNamedStoredProcedureQuery("leavePendingRoutines")
					.setParameter("actionType", getPLeaveType)
		...
		logger.info("Method : DAO getPLeaveType ends");	
		return response;
	}
	...
	public ResponseEntity<JsonResponse<List<RestLeaveApplyModel>>> getPLApplyData(
		...
		logger.info("Method : RestLeavePendingDao details starts");
		...
		try {		 
			List<Object[]> x = em.createNamedStoredProcedureQuery("leavePendingRoutines")
					.setParameter("actionType", "viewPLApplied")
		...
		logger.info("Method : RestLeavePendingDao ends");
	    return response;
	}
	 ...
	 */
 	@SuppressWarnings("unchecked")
	public ResponseEntity<JsonResponse<RestLeaveApplyModel>> getLeavePendingById(String id,String action) {
