...
public class AssetPolicyMasterDao {
	Logger logger = LoggerFactory.getLogger(AssetPolicyMasterDao.class);
	...
	@SuppressWarnings("unchecked")
	public List<DropDownModel> getServiceTypeForPolicy() {
		logger.info("Method : getServiceTypeForPolicy starts");
		
		List<DropDownModel> serviceTypeList = new ArrayList<DropDownModel>();
		
		try {
			List<Object[]> x = em.createNamedStoredProcedureQuery("assetPolicy")
					.setParameter("actionType", "getServiceType").setParameter("actionValue", "").getResultList();
		...
		logger.info("Method : getServiceTypeForPolicy ends");
	...
	public ResponseEntity<JsonResponse<Object>> addPolicyDao(List<AssetPolicyMaster> assetPolicyMaster) {

		logger.info("Method in Dao: addPolicyDao starts");
		...
		logger.info("Method in Dao: addPolicyDao ends");
	 ...
	 */
	@SuppressWarnings("unchecked")
	public ResponseEntity<JsonResponse<List<AssetPolicyMaster>>> getServiceDetailsDao(String itemId) {

		logger.info("Method : getServiceDetailsDao starts");
		...
		try {
			...
			for (Object[] m : x) {
				AssetPolicyMaster saleOder = new AssetPolicyMaster(m[0], m[1], m[2], m[3], m[4], m[5], m[6], m[7],m[8],null);
		...
		logger.info("Method : getServiceDetailsDao ends");
		return response;
	}
